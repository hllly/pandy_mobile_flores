...
StateRL::StateRL(CtrlComponent &ctrl_component, const std::string &config_path,
    ...
    // }
    std::chrono::time_point<std::chrono::high_resolution_clock> last_execution_time = std::chrono::high_resolution_clock::now();
        ...
        while (true) {
            try {
                executeAndSleep(
                    ...
                    // std::cout<<"hz!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! : "<<ctrl_comp_.frequency_ / params_.decimation<<std::endl;

                // // 记录当前时间
                // auto current_execution_time = std::chrono::high_resolution_clock::now();
                // // 计算并打印两次执行之间的时间间隔
                // auto interval = current_execution_time - last_execution_time;
                // double interval_seconds = std::chrono::duration_cast<std::chrono::duration<double>>(interval).count();
                // std::cout << "Time interval between executions: " << 1/interval_seconds << " hz" << std::endl;
                // // 更新上一次执行的时间
    ...
    // std::chrono::time_point<std::chrono::high_resolution_clock> StateRL::last_call_time = std::chrono::high_resolution_clock::now();
}
...
void StateRL::enter() {
    ...
    for (int i = 0; i < params_.num_of_dofs; ++i) {
        ...
        robot_command_.motor_command.kp[i] = 500;
        robot_command_.motor_command.kd[i] = 5;
        robot_command_.motor_command.tau[i] = 0;
    }

    running_ = true;

    this->node_ =  std::make_shared<rclcpp::Node>("rl_debug_node");
    rl_test_publisher_ = this->node_->create_publisher<pd_interfaces::msg::RlTest>("RlTest", 1);
}
...
FSMStateName StateRL::checkChange() {
    ...
    for (int i = 0; i < 16; i++) {
        if (robot_state_.motor_state.error[i] != 0) {
            has_error = true;
            std::cout << "motor_state_error[" << i << "] : " 
                     << robot_state_.motor_state.error[i] << std::endl;
        }
    }
...
torch::Tensor StateRL::computeObservation() {
    ...
    for (const std::string &observation: params_.observations) {
        if (observation == "lin_vel") {
            // std::cout<<"obs_.lin_vel : "<<obs_.lin_vel<<std::endl;
            obs_list.push_back(obs_.lin_vel * params_.lin_vel_scale);
        } else if (observation == "ang_vel") {
            // at::Tensor ang_vel;
            // ang_vel = quatRotateInverse(obs_.base_quat, obs_.ang_vel, params_.framework);
            // std::cout<<"obs_.ang_vel : "<<obs_.ang_vel<<std::endl;
            // obs_list.push_back(
            ...
            dof_pos_copy.index_put_({0, 11}, 0); 
            dof_pos_copy.index_put_({0, 15}, 0);
            // std::cout<<"obs_.dof_pos : "<<dof_pos_copy<<std::endl;
            obs_list.push_back((dof_pos_copy - params_.default_dof_pos) * params_.dof_pos_scale);
...
void StateRL::loadYaml(const std::string &config_path) {
    ...
    // printf("params_.action_scale : %f\n", params_.action_scale);
...
torch::Tensor StateRL::forward() {
    ...
    if (!params_.observations_history.empty()) {
        ...
        // std::cout << "observations_history not empty" << std::endl;
...
void StateRL::getState() {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        robot_state_.motor_state.error[i] = ctrl_comp_.joint_error_state_interface_[i].get().get_value();
        // std::cout<<"robot_state_.motor_state.q["<<i<<"] : "<<robot_state_.motor_state.q[i]<<std::endl;
        if(robot_state_.motor_state.error[i] !=0){
            std::cout<<"motor_state_error["<<i<<"] : "<<robot_state_.motor_state.error[i]<<std::endl;
        }
    }
    
    if(ctrl_comp_.control_inputs_.ly != control_.x 
        || ctrl_comp_.control_inputs_.lx != -control_.y 
         || ctrl_comp_.control_inputs_.rx != -control_.yaw){
        std::cout<<"control_.x : "<<ctrl_comp_.control_inputs_.ly<<"  "
        <<"control_.y : "<<ctrl_comp_.control_inputs_.lx<<"  "
        <<"control_.yaw : "<<ctrl_comp_.control_inputs_.rx<<std::endl;
    }
...
void StateRL::runModel() {
    ...
    // std::cout<<"output_actions_ : "<<clamped_actions<<std::endl;
    ...
    // std::cout<<"params_.action_scale : "<<params_.action_scale<<std::endl;
    ...
    // output_dof_pos_ = output_dof_pos_ - obs_.dof_pos;
    // std::cout<<"output_dof_pos_ : "<<output_dof_pos_<<std::endl;
    output_dof_vel_[0][3] = actions_scaled[0][3].item<double>();
    output_dof_vel_[0][7] = actions_scaled[0][7].item<double>();
    output_dof_vel_[0][11] = actions_scaled[0][11].item<double>();
    output_dof_vel_[0][15] = actions_scaled[0][15].item<double>();

    for (int i = 0; i < params_.num_of_dofs; ++i) {
        double torque = params_.rl_kp[0][i].item<double>() * (output_dof_pos_[0][i].item<double>() - obs_.dof_pos[0][i].item<double>()) +
                           params_.rl_kd[0][i].item<double>() * (output_dof_vel_[0][i].item<double>() - obs_.dof_vel[0][i].item<double>());
        if (torque > params_.torque_limits[0][i].item<double>()) {
            robot_command_.motor_command.tau[i] = params_.torque_limits[0][i].item<double>();
            robot_command_.motor_command.kp[i] = 0;
            robot_command_.motor_command.kd[i] = 0;
        } else if (torque < -params_.torque_limits[0][i].item<double>()) {
            robot_command_.motor_command.tau[i] = -params_.torque_limits[0][i].item<double>();
            robot_command_.motor_command.kp[i] = 0;
            robot_command_.motor_command.kd[i] = 0;
        } else{
...
void StateRL::publishDebug(){
    for (int i =0; i<16; i++){
        msg.obs_joint_pos[i] = obs_.dof_pos[0][i].item<double>();
        msg.obs_joint_vel[i] = obs_.dof_vel[0][i].item<double>();
        msg.obs_action[i] = obs_.actions[0][i].item<double>();
        msg.output_dof_pos[i] = output_dof_pos_[0][i].item<double>();
    }

    // auto msg.ang_vel = ;
    // auto msg.base_quaternion[i] = obs_.base_quat;
    for (int i = 0; i < 4; i++) {
        msg.obs_base_quaternion[i] = obs_.base_quat[0][i].item<double>();
    }

    for (int i = 0; i < 3; i++) {
        msg.obs_ang_vel[i] = obs_.ang_vel[0][i].item<double>();
    }
    
    this->rl_test_publisher_->publish(msg);
}
