...
template <typename T>
inline Vector<T> Vector<T>::extract(
...
template <typename T>
inline unsigned int partition(Vector<T>& v, unsigned int begin,
                              unsigned int end) {
...
class Matrix {
 public:
  ...
  inline Matrix<T> extract(const std::set<unsigned int>& r_indexes,
  ...
  inline void setRows(const std::set<unsigned int>& indexes,
                      const Matrix<T>& m);
...
template <typename T>
Matrix<T>::Matrix() : n(0), m(0), v(0) {}

template <typename T>
Matrix<T>::Matrix(unsigned int n, unsigned int m) : v(new T*[n]) {
  this->n = n;
  this->m = m;
  v[0] = new T[m * n];
  for (unsigned int i = 1; i < n; i++) v[i] = v[i - 1] + m;
}

template <typename T>
Matrix<T>::Matrix(const T& a, unsigned int n, unsigned int m) : v(new T*[n]) {
  this->n = n;
  this->m = m;
  v[0] = new T[m * n];
  for (unsigned int i = 1; i < n; i++) v[i] = v[i - 1] + m;
...
template <class T>
Matrix<T>::Matrix(MType t, const T& a, const T& o, unsigned int n,
                  unsigned int m)
    : v(new T*[n]) {
  this->n = n;
  this->m = m;
...
template <class T>
Matrix<T>::Matrix(MType t, const Vector<T>& a, const T& o, unsigned int n,
                  unsigned int m)
    : v(new T*[n]) {
  this->n = n;
  this->m = m;
...
template <typename T>
Matrix<T>::Matrix(const Matrix<T>& rhs) : v(new T*[rhs.n]) {
  n = rhs.n;
  m = rhs.m;
  v[0] = new T[m * n];
  for (unsigned int i = 1; i < n; i++) v[i] = v[i - 1] + m;
...
template <typename T>
inline void Matrix<T>::resize(const T& a, const unsigned int n,
                              const unsigned int m) {
  resize(n, m);
  for (unsigned int i = 0; i < n; i++)
    for (unsigned int j = 0; j < m; j++) v[i][j] = a;
...
inline Vector<T> Matrix<T>::extractColumn(const unsigned int j) const {
  ...
  for (unsigned int i = 0; i < n; i++) tmp[i] = v[i][j];
...
inline Vector<T> Matrix<T>::extractDiag() const {
  ...
  for (unsigned int i = 0; i < d; i++) tmp[i] = v[i][i];

  return tmp;
}

template <typename T>
inline Matrix<T> Matrix<T>::extractRows(
    const std::set<unsigned int>& indexes) const {
...
template <typename T>
inline Matrix<T> Matrix<T>::extractColumns(
    const std::set<unsigned int>& indexes) const {
...
template <typename T>
inline void Matrix<T>::setRows(const std::set<unsigned int>& indexes,
                               const Matrix<T>& m) {
  unsigned int i = 0;
...
template <typename T>
inline void Matrix<T>::setColumns(const std::set<unsigned int>& indexes,
                                  const Matrix<T>& a) {
  unsigned int j = 0;
...
template <typename T>
inline void Matrix<T>::set(const std::set<unsigned int>& r_indexes,
...
Matrix<T> pinv(const Matrix<T>& A) {
  ...
  for (unsigned int i = 0; i < A.nrows(); i++) {
    e.reset(i);
    tmp.setColumn(
        i, dot_prod(dot_prod(dot_prod(V, Matrix<double>(DIAG, 1.0 / W, 0.0,
                                                        W.size(), W.size())),
                             t(U)),
                    e));
  }
...
template <typename T>
Vector<T> lu_solve(const Matrix<T>& LU, const Vector<T>& b,
...
template <typename T>
void lu_solve(const Matrix<T>& LU, Vector<T>& x, const Vector<T>& b,
              Vector<unsigned int>& index) {
...
void cholesky(const Matrix<T> A, Matrix<T>& LL) {
  ...
  for (unsigned int i = 0; i < n; i++) {
    for (unsigned int j = i; j < n; j++) {
      sum = LL[i][j];
      for (int k = i - 1; k >= 0; k--) sum -= LL[i][k] * LL[j][k];
      if (i == j) {
        if (sum <= 0.0)
          throw std::logic_error(
    ...
    for (unsigned int k = i + 1; k < n; k++) LL[i][k] = LL[k][i];
  }
}
...
void forward_elimination(const Matrix<T>& L, Vector<T>& y, const Vector<T> b) {
  ...
  for (unsigned int i = 1; i < n; i++) {
    y[i] = b[i];
    for (unsigned int j = 0; j < i; j++) y[i] -= L[i][j] * y[j];
    y[i] = y[i] / L[i][i];
  }
}
...
template <typename T>
void backward_elimination(const Matrix<T>& U, Vector<T>& x,
                          const Vector<T>& y) {
  if (U.ncols() != U.nrows())
    throw std::logic_error(
