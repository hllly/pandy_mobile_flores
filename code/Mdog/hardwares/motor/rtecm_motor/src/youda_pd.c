...
#include "pd_interfaces/msg/low_cmd.h"
#include "pd_interfaces/msg/low_state.h"
#include "pd_interfaces/msg/motor_cmd.h"
#include "pd_interfaces/msg/motor_state.h"

// #include "interface/sharedMemory.h"

// SharedMemoryObject<MobileBaseSharedMemoryMessage> sharedMemory;
// sharedMemory.createNew(MDOG_SHARED_MEMORY, true);
// sharedMemory().init();
...
pd_interfaces__msg__LowCmd motorCMD_msg;
...
pd_interfaces__msg__LowState message;
...
const pd_interfaces__msg__LowCmd * msg = (const pd_interfaces__msg__LowCmd *)msgin;
// printf("motorCMD_subscriber_callback\n");
// for (size_t i = 0; i < 16; i++)
// {
//   msg->motor_cmd[i].tau=i+10;
// }

 
       uint32_t len = chry_ringbuffer_write(&rb, msgin, sizeof(pd_interfaces__msg__LowCmd));

         if(len == sizeof(pd_interfaces__msg__LowCmd)){
...
uint16_t getCW2(uint8 nSevoOn , uint16_t u16Sate, uint16_t *u16Ctrol){
...
void redtest(char *ifname, int argc, const char * argv[])
{
  ...
  const char * topic_name = "LowState";
  const rosidl_message_type_support_t * motorState_type_support =
    ROSIDL_GET_MSG_TYPE_SUPPORT(pd_interfaces, msg, LowState);
...
//#################################################################################
  ...
  rc = rclc_subscription_init_default(
    &motorCMD_sub,
    &my_node,
     ROSIDL_GET_MSG_TYPE_SUPPORT(pd_interfaces, msg, LowCmd),
    topic_name2);
  if (rc != RCL_RET_OK) {
...
//   custom_interfaces__msg__DrivingCommand__init(&motorCMD_msg);
  pd_interfaces__msg__LowCmd__init(&motorCMD_msg);
...
//   }
    ...
    pd_interfaces__msg__LowState__init(&message);
               ...
               if (received == 1)
               {
                 motorStatePub();
                  ...
                  if (printCnt % 100 == 0)
                  {
                     ...
                     {
                        printf("slave: %d cw: %5d state: %5d pos: %d vel: %9f taget_torque:%9d || torque: %9d error:%4x mode:%d", i, youda_target[i]->status, youda_val[i]->status, youda_val[i]->Pos, youda_val[i]->vel/ENCODER_17BIT, youda_target[i]->tTorque, youda_val[i]->Torque,youda_val[i]->error,youda_val[i]->opMode);
                     }
                  ...
                  txData[3] = youda_val[0]->Pos/ENCODER_17BIT; // 131072.0;
                  txData[4] = youda_val[0]->vel/ENCODER_17BIT;// ENCODER_17BIT_rpm ;
...
OSAL_THREAD_FUNC_RT ecatthread(void *ptr)
{
   ...
   uint8 motorEnable[16];
   memset(motorEnable, 0, sizeof(motorEnable));
   memset(readBuffer, 0, sizeof(readBuffer));
   pd_interfaces__msg__LowCmd *LowCmdPtr=readBuffer;
   pd_interfaces__msg__MotorCmd *MotorCmdPtr=LowCmdPtr->motor_cmd;

   pd_interfaces__msg__MotorState *motorPtr;

   // double torque_const[16]= {1.1577,1.1577,1.1577,1.162,
   //                             1.1577,1.1577,1.1577,1.162,
   //                             1.1577,1.1577,1.1577,1.162,
   //                             1.1577,1.1577,1.1577,1.162,};
   double torque_const[16]= { 0.11577,0.11577,0.11577,0.1162,
                              0.11577,0.11577,0.11577,0.1162,
                              0.11577,0.11577,0.11577,0.1162,
                              0.11577,0.11577,0.11577,0.1162,};

   double max_current[16]= { 40.0,40.0,40.0,10.0,
                              40.0,40.0,40.0,10.0,
                              40.0,40.0,40.0,10.0,
                              40.0,40.0,40.0,10.0,};
   ...
   while (1)
   {
      if (dorun > 0)
      {
         ...
         if (inOP)
         {
            ...
            uint32_t len = chry_ringbuffer_read(&rb, readBuffer, sizeof(pd_interfaces__msg__LowCmd));
            if(len){
               
               //  printf("Received %d bytes from ringbuffer\n", len);
               // printf posE velE kp kd 
               // printf("kp: %f\n", MotorCmdPtr[0].kp);
               // printf("kd: %f\n", MotorCmdPtr[0].kd);
               // printf("posE: %f\n",posE);
               // printf("velE: %f\n",velE);
               ...
               //  {
               //     printf("tau: %f\n", MotorCmdPtr[i].tau);
               //     motorEnable[i]=MotorCmdPtr[i].mode;
               //     printf("mode: %d\n", motorEnable[i]);
               //  }
            ...
            {
               youda_target[cnt]->opMode = 10;
              
               getCW2(MotorCmdPtr[cnt].mode, youda_val[cnt]->status, &youda_target[cnt]->status);
               ...
               posE = MotorCmdPtr[cnt].q - youda_val[cnt]->Pos/ENCODER_17BIT;
               velE = MotorCmdPtr[cnt].dq - youda_val[cnt]->vel/ENCODER_17BIT;

                youda_target[cnt]->tTorque = (int16_t)((MotorCmdPtr[cnt].tau+posE*MotorCmdPtr[cnt].kp/100+velE*MotorCmdPtr[cnt].kd/100)*1000.0/(torque_const[cnt]*max_current[cnt]));
               //  youda_target[cnt]->tTorque = (int16_t)(MotorCmdPtr[cnt].tau+posE*MotorCmdPtr[cnt].kp+velE*MotorCmdPtr[cnt].kd);
 
                currentLimit = (int16_t)MotorCmdPtr[cnt].tlimit;
                  ...
                  motorPtr = &message.motor_state[cnt];
        
                  motorPtr->q=youda_val[cnt]->Pos/ENCODER_17BIT;
                  motorPtr->dq=youda_val[cnt]->vel/ENCODER_17BIT;
                  motorPtr->tau=youda_val[cnt]->Torque;
                  motorPtr->error=youda_val[cnt]->error;
                  motorPtr->mode=MotorCmdPtr[cnt].mode;
                  motorPtr->kp=MotorCmdPtr[cnt].kp;
                  motorPtr->kd=MotorCmdPtr[cnt].kd;
                  // printf("motor[%d]kd:%f \n",cnt,motorPtr->kd);
            ...
            // for (size_t i = 0; i < youda_cnt; i++)
            // {
            //   motorPtr = &message.motor_state[i];
        
            //   motorPtr->q=youda_val[i]->Pos/ENCODER_17BIT;
            //    motorPtr->dq=youda_val[i]->vel/ENCODER_17BIT;
            //    motorPtr->tau=youda_val[i]->Torque;
            //    motorPtr->error=youda_val[i]->error;
            //    motorPtr->mode=MotorCmdPtr[i].mode;
            //    motorPtr->kp=MotorCmdPtr[i].kp;
            //    motorPtr->kd=MotorCmdPtr[i].kd;
...
int main(int argc, char *argv[])
{
   ...
   // updinit(&fd, "127.0.0.1", 8888); // init udp tools to record data
