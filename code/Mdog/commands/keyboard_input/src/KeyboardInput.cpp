...
void KeyboardInput::check_value(char key) {
    switch (key) {
        ...
        case 'W':
            ...
            // std::cout<< "inputs_.ly : "<<inputs_.ly<<std::endl;
            break;
        case 's':
        case 'S':
            inputs_.ly = max<float>(inputs_.ly - sensitivity_left_, -3.0);
            break;
        case 'd':
        case 'D':
            inputs_.lx = min<float>(inputs_.lx + sensitivity_left_, 2.0);
            break;
        case 'a':
        case 'A':
            inputs_.lx = max<float>(inputs_.lx - sensitivity_left_, -2.0);
            break;
        ...
        case 'l':
        case 'L':
            inputs_.rx = min<float>(inputs_.rx + sensitivity_right_, 2.0);
            break;
        case 'j':
        case 'J':
            inputs_.rx = max<float>(inputs_.rx - sensitivity_right_, -2.0);
            break;
        default:
            break;
    }
